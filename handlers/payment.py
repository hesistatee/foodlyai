...
from utils.keyboards import choose_analyze_kb, get_tariffs_keyboard
    ...
    if order:
        ...
        if user and tariff:
            await user_repo.update_subscription(
                telegram_id=message.from_user.id, days=tariff.days
            )
            logger.info(
                f"Subscription updated for user {message.from_user.id}, added {tariff.days} days"
            )
            ...
            logger.error(
                f"User {message.from_user.id} or tariff {order.tariff_id} not found after payment"
            )
...
async def unsuccessful_payment(message: Message, session: AsyncSession):
    ...
    order_repo = OrderRepository(session)
    await order_repo.update_order_status(order_id=order_id, status=OrderStatus.FAILED)
